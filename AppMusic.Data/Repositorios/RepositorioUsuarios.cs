...
namespace JMusik.Data.Repositorios
{
    public class RepositorioUsuarios : IUsuariosRepositorio
    {
        ...
        {
            //obtiene usuario por medio del username
            ...
            if(usuarioBd != null)
            {
                try
                {
                    //si existe, verifica el password del usuario con el que le está pasando
                    var resultado = _passwordHasher.VerifyHashedPassword(usuarioBd, usuarioBd.Password, datosLoginUsuario.Password);
                    //si es exitoso, retorna true y el usuario, de lo contrario false.
                    return (resultado == PasswordVerificationResult.Success ? true : false, usuarioBd);
                }
                catch (Exception e)
                {
                    _logger.LogError($"Error en {nameof(ValidarDatosLogin)}:" + e.Message);
                }
            }
            return (false, null);
        }
